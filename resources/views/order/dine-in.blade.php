...
@section('title', 'Pesanan Dine In - Toko Roti Cloud Bread')
@section('content')

<!-- Hero Section Dine In -->
<header class="relative w-full min-h-[40vh] flex flex-col justify-center overflow-hidden">
    <!-- Background Image -->
    <div class="absolute inset-0 z-0 w-full h-full">
        <img 
            src="{{ asset('assets/images/Hero.jpg') }}" 
            alt="Dine In Cloud Bread"
    ...
    <div class="relative z-10 max-w-7xl mx-auto w-full px-6 md:px-12 py-16">
        <div class="max-w-2xl">
            ...
            <h1 class="font-display-lg text-4xl md:text-6xl text-white leading-tight font-bold drop-shadow-lg mt-6">
                Pesan & <br>
                <span class="text-tertiary-fixed-dim italic font-serif drop-shadow-lg">Nikmati</span> di Tempat
            </h1>
            <p class="mt-4 font-body-lg text-lg text-white/90 max-w-lg leading-relaxed drop-shadow-md">
                Pilih menu favorit Anda, tentukan meja, dan kami akan segera menyiapkan pesanan Anda dengan penuh cinta.
            </p>
        </div>
    </div>
</header>
...
<!-- Dine In Content -->
<div class="relative bg-gradient-to-b from-[#f5f0e8] to-[#faf8f5] py-12 px-4 sm:px-6 lg:px-8" id="dine-in-app" data-menus="{{ json_encode($menus) }}" data-csrf="{{ csrf_token() }}">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white/80 backdrop-blur-md rounded-3xl border border-[#ebdcb9]/40 p-6 shadow-sm">
                <h2 class="text-lg font-bold text-neutral-800 mb-4 flex items-center gap-2" style="font-family: 'Outfit', sans-serif;">
                    <svg class="w-5 h-5 text-[#8B6B4A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="customer-name" placeholder="Masukkan nama Anda..." class="w-full px-4 py-3 rounded-xl border border-neutral-200 focus:outline-none focus:ring-2 focus:ring-[#8B6B4A]/20 focus:border-[#8B6B4A] transition-all bg-neutral-50/50">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Email Pelanggan</label>
                        <input type="email" id="customer-email" placeholder="nama@email.com..." class="w-full px-4 py-3 rounded-xl border border-neutral-200 focus:outline-none focus:ring-2 focus:ring-[#8B6B4A]/20 focus:border-[#8B6B4A] transition-all bg-neutral-50/50">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Pilih Nomor Meja</label>
                        <select id="table-number" class="w-full px-4 py-3 rounded-xl border border-neutral-200 focus:outline-none focus:ring-2 focus:ring-[#8B6B4A]/20 focus:border-[#8B6B4A] transition-all bg-neutral-50/50">
            ...
            <div class="flex flex-col sm:flex-row gap-4 items-center justify-between">
                <div class="flex gap-2 overflow-x-auto pb-1 w-full sm:w-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                    <button onclick="filterCategory('all')" id="cat-all" class="px-5 py-2.5 rounded-xl text-sm font-semibold transition-all bg-[#8B6B4A] text-white shadow-[0_4px_12px_rgba(139,107,74,0.2)]">
                        Semua
                    </button>
                    <button onclick="filterCategory('Roti Klasik')" id="cat-roti-klasik" class="px-5 py-2.5 rounded-xl text-sm font-semibold transition-all bg-white text-neutral-600 border border-[#ebdcb9]/30 hover:bg-[#f5f3ef] hover:text-[#8B6B4A]">
                        Roti Klasik
                    </button>
                    <button onclick="filterCategory('Donat')" id="cat-donat" class="px-5 py-2.5 rounded-xl text-sm font-semibold transition-all bg-white text-neutral-600 border border-[#ebdcb9]/30 hover:bg-[#f5f3ef] hover:text-[#8B6B4A]">
                        Donat
                    </button>
                    <button onclick="filterCategory('Pastry')" id="cat-pastry" class="px-5 py-2.5 rounded-xl text-sm font-semibold transition-all bg-white text-neutral-600 border border-[#ebdcb9]/30 hover:bg-[#f5f3ef] hover:text-[#8B6B4A]">
                        Pastry
                    </button>
                </div>
                
                <div class="relative w-full sm:w-72">
                    <input type="text" id="search-menu" oninput="searchMenu()" placeholder="Cari menu..." class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-neutral-200 focus:outline-none focus:ring-2 focus:ring-[#8B6B4A]/20 focus:border-[#8B6B4A] transition-all text-sm">
                ...
                @foreach ($menus as $menu)
                    <div class="product-card bg-white rounded-3xl border border-[#ebdcb9]/30 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 flex flex-col group" 
                        ...
                        <div class="h-44 w-full overflow-hidden relative bg-neutral-100">
                            ...
                            <span class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm text-xs font-bold text-[#8B6B4A] px-3 py-1 rounded-full shadow-sm">
                                Rp {{ number_format($menu->price, 0, ',', '.') }}
                            </span>
                        </div>
                        <div class="p-5 flex-grow flex flex-col justify-between">
                            <div>
                                <h3 class="font-bold text-neutral-800 group-hover:text-[#8B6B4A] transition-colors">{{ $menu->name }}</h3>
                                <p class="text-xs text-neutral-500 mt-2 leading-relaxed line-clamp-2">{{ $menu->description }}</p>
                            </div>
                            <button onclick="addToCart({{ $menu->id }})" class="w-full mt-4 bg-neutral-50 hover:bg-[#8B6B4A] group-hover:bg-[#8B6B4A]/5 group-hover:hover:bg-[#8B6B4A] text-[#8B6B4A] hover:text-white group-hover:text-[#8B6B4A] group-hover:hover:text-white py-2.5 rounded-xl text-xs font-bold transition-all border border-[#8B6B4A]/10 hover:border-transparent flex items-center justify-center gap-1.5">
        ...
        <div class="space-y-6 lg:sticky lg:top-24">
            ...
            <div class="bg-white/80 backdrop-blur-md rounded-3xl border border-[#ebdcb9]/40 p-6 shadow-sm">
                <div class="flex items-center justify-between border-b border-neutral-100 pb-4 mb-4">
                    ...
                    <span id="cart-count" class="bg-[#8B6B4A]/10 text-[#8B6B4A] text-xs font-bold px-2.5 py-1 rounded-full">
                        0 item
                    </span>
                </div>
                ...
                <div id="cart-totals-section" class="border-t border-neutral-100 mt-4 pt-4 space-y-3 hidden">
                    ...
                    <div class="flex justify-between text-base font-bold text-neutral-800 border-t border-dashed border-neutral-200 pt-3">
                        ...
                        <span id="cart-total" class="text-[#8B6B4A]">Rp 0</span>
                    </div>
                    ...
                    <div class="space-y-2 mt-4">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <label class="flex flex-col items-center justify-center p-3 rounded-xl border border-neutral-200 cursor-pointer hover:bg-neutral-50 transition-all text-center has-[:checked]:border-[#8B6B4A] has-[:checked]:bg-[#8B6B4A]/5 has-[:checked]:ring-1 has-[:checked]:ring-[#8B6B4A]">
                                <input type="radio" name="payment-method" value="cash" checked class="peer hidden">
                                <span class="text-xs font-bold text-neutral-700 peer-checked:text-[#8B6B4A]">Tunai</span>
                                <span class="text-[9px] text-neutral-400 mt-0.5 peer-checked:text-[#8B6B4A]/80">di Kasir</span>
                            </label>
                            <label class="flex flex-col items-center justify-center p-3 rounded-xl border border-neutral-200 cursor-pointer hover:bg-neutral-50 transition-all text-center has-[:checked]:border-[#8B6B4A] has-[:checked]:bg-[#8B6B4A]/5 has-[:checked]:ring-1 has-[:checked]:ring-[#8B6B4A]">
                                <input type="radio" name="payment-method" value="qris" class="peer hidden">
                                <span class="text-xs font-bold text-neutral-700 peer-checked:text-[#8B6B4A]">QRIS</span>
                                <span class="text-[9px] text-neutral-400 mt-0.5 peer-checked:text-[#8B6B4A]/80">Instant</span>
                            </label>
                            <label class="flex flex-col items-center justify-center p-3 rounded-xl border border-neutral-200 cursor-pointer hover:bg-neutral-50 transition-all text-center has-[:checked]:border-[#8B6B4A] has-[:checked]:bg-[#8B6B4A]/5 has-[:checked]:ring-1 has-[:checked]:ring-[#8B6B4A]">
                                <input type="radio" name="payment-method" value="transfer" class="peer hidden">
                                <span class="text-xs font-bold text-neutral-700 peer-checked:text-[#8B6B4A]">Transfer</span>
                                <span class="text-[9px] text-neutral-400 mt-0.5 peer-checked:text-[#8B6B4A]/80">Bank</span>
                            </label>
                        </div>
                    </div>

                    <!-- Tombol Pesan Sekarang -->
                    <button id="btn-checkout" onclick="processCheckout()" class="w-full mt-4 bg-[#8B6B4A] hover:bg-[#7A5D3F] text-white py-4 rounded-2xl font-bold transition-all shadow-md flex items-center justify-center gap-2">
                        Pesan Sekarang
            ...
            <div class="bg-white/80 backdrop-blur-md rounded-3xl border border-[#ebdcb9]/40 p-5 shadow-sm">
                <button onclick="toggleHistoryModal()" class="w-full flex items-center justify-between text-neutral-600 hover:text-[#8B6B4A] transition-all font-semibold text-sm">
...
<div id="receipt-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm hidden opacity-0 transition-opacity duration-300 print:absolute print:inset-0 print:flex print:bg-transparent print:backdrop-blur-none">
    <div class="bg-white rounded-3xl w-full max-w-md mx-4 overflow-hidden shadow-2xl border border-neutral-100 transform scale-95 transition-transform duration-300 print:shadow-none print:border-none print:m-0 print:w-full print:max-w-none">
        <div class="bg-[#8B6B4A] text-white p-6 text-center relative">
        ...
        <div class="p-6 border-t border-neutral-100 flex gap-3 print:hidden">
            ...
            <button onclick="window.print()" class="flex-1 px-4 py-3 rounded-xl bg-[#8B6B4A] hover:bg-[#7A5D3F] text-white font-bold transition-all text-center text-sm flex items-center justify-center gap-1.5 shadow-sm">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
